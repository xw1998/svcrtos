/**
* @brief SVCrtOS 板级移植层 - STM32F401
* @details 实现 port 层要求板级提供的基础配置：
*          - FPU 使能（CPACR + FPCCR）
*          - NVIC 优先级分组与内核异常优先级
*          - 板载设备注册（COM1 = USART2 / LED = PC13）
*          - CPU 异常向量与系统节拍入口
...
{
    /* F401 板上只有一颗 LED（PC13），因此只登记 "LED" 一个名字；
     * 设备接口的形状与 F427 板一致，App 侧代码不用改。 */
    svcrt_dev_register("LED",  &led_drv, LED_ID_BOARD);
