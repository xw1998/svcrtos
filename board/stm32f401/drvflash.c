/**
* @file drvflash.c
* @brief STM32F401 片内 Flash 擦写驱动实现（HAL 库）
* @details STM32F401（512KB）的扇区划分：
*          - 扇区 0~3 ：16KB
*          - 扇区 4   ：64KB
*          - 扇区 5~7 ：128KB
*          扇区号由地址公式推出，并把上界卡在 CHIP_FLASH_SIZE 上，
*          因此同一份实现也适用于 1MB/2MB 的 F4（扇区 5~11/23）。
*
...
static uint32 svcrt_flash_sector_index(uint32 addr)
{
     ...
     * 不挡的话下面按公式会算出 5~23 的“合法”扇区号（F401 512KB 只到 7，1MB F4 到 11），
     * 越界扇区被交给 HAL 去擦。 */
