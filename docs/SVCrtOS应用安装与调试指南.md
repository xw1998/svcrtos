...
> 配置区与两种安装策略的原理，以及镜像格式的设计动因，见《配置区与安装策略.md》。
...
**安装路径与上电扫描必须给出同一个窗口**。两条路都按镜像头部声明的
`ram_size`（向上取整到 2 的幂）去绑 RAM 块；固定槽在配置里登记的 `ram_size`
只是这块窗口的**上限**，镜像要得比它大时安装直接报错。若安装路径改用配置值
绑窗口，同一镜像「装完立刻启动」与「重启后再扫描」会拿到两个不同大小的窗口，
栈的位置随之相差一个窗口差——而栈溢出的判据（保存的 PSP < 栈底）是按这个
窗口算的，于是会按错的阈值生效。现场表现为「装完的同一上电周期内 App 早停，
`fault` 里一条 `STACKOVF`，重启后却完全正常」。
...
- **`SHELL_ENABLE=0` 时由配置点名落点**：没有命令行就没有"先擦再喊"的时机，所以用
  `INSTALLER_FIXED_SLOT`（配置槽序号，默认 `-1`）把目标槽告诉常驻安装任务。设成 N 后，
  它在上电后、开窗前先做准备：该槽里若有正在运行的镜像，先停掉它（`svcrt_loader_stop()`，
  等价于控制台上的 `app stop <slot>`；否则擦除会擦掉正在取指的代码），再整槽擦一次并打印
  `fixed slot N cleared, send the file now`；一个上电周期只接受一帧，第二帧被拒并回
  `one image already installed this boot: reboot before sending another one`，要再装一次
  就重启。保持 `-1` 时，常驻任务在 fixed 模式下拒绝任何帧，而不是猜一个槽去擦。
