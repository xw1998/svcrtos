...
> **开启内核 Shell 后，串口读权归控制台。** 默认 `SHELL_ENABLE = 1`，内核不再注册常驻
> 安装任务；先在控制台敲 `install` 打开一次性接收窗口，再发送镜像，避免两个读者把同一个
> 串口 FIFO 的字节流随机分掉。只有 `SHELL_ENABLE = 0` 时才是“上电即在收”的常驻安装任务。
> 完整命令与示例见 [`docs/内核Shell控制台使用说明.md`](内核Shell控制台使用说明.md)。
