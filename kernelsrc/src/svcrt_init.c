...
*          当应用程序（如 CubeMX 生成的 main.c）定义了自己的 main() 时，
*          此弱定义会被覆盖，由应用程序控制完整启动流程。
*
*          应用程序应在自己的 main() 中按以下顺序调用内核初始化：
*          1) HAL库初始化 + 时钟配置 + 外设初始化（CubeMX生成）
*          2) svcrt_port_board_init()
*          3) svcrt_cfg_load()
*          4) svcrt_register_tasks()（用户自定义）
*          5) svcrt_port_irq_init()
*          6) svcrt_kernel_init()
*          7) svcrt_start_idle()（永不返回）
*
* @author xw
* @date 2026.05.03
*/
