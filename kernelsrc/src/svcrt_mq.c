...
/* ¼ÆËã¡°±»»½ĞÑºóÖØĞÂÅÅ¶Ó¡±¿ÉÓÃµÄÊ£Óà³¬Ê±£¨ms£©¡£
 * deadline==0 ±íÊ¾ÎŞÏŞµÈ´ı£¬·µ»Ø -1£»ÒÑÓÃ¾¡·µ»Ø 0¡£ */
...
int32 svcrt_mq_send_internal(int32 handle, uint32 *buf, int32 len_words, int32 timeout_ms)
{
    ...
    /* ¼ÇÂ¼ÆğÊ¼½ÚÅÄÓë³¬Ê±ÉÏÏŞ£º±»»½ĞÑºóÈô¿Õ¼äÓÖ±»±ğÈËÇÀ×ß£¬ĞèÒª°´Ê£ÓàÊ±¼äÖØÊÔ */
    ...
    /* ±»»½ĞÑ²»µÈÓÚÒ»¶¨ÄÃµ½¿Õ¼ä£º»½ĞÑºó¿Õ¼ä¿ÉÄÜÒÑ±»ÆäËü·¢ËÍÕßÕ¼×ß¡£
     * Ô­ÊµÏÖ´ËÊ±Ö±½Ó·µ»Ø -1£¬ÏûÏ¢±»ÇÄÇÄ¶ªµô£»ÕâÀï°´Ê£ÓàÊ±¼äÖØĞÂÅÅ¶ÓµÈ´ı¡£ */
    while(p_mq->count >= SVCRT_MQ_DEPTH)
    {
        ...
        ret = svcrt_task_block_in_critical((uint32)remain);     /* ¹ØÖĞ¶Ï·µ»Ø */
...
int32 svcrt_mq_recv_internal(int32 handle, uint32 *buf, int32 len_words, int32 timeout_ms)
{
    ...
    /* ¼ÇÂ¼ÆğÊ¼½ÚÅÄÓë³¬Ê±ÉÏÏŞ£º±»»½ĞÑºóÈôÏûÏ¢ÓÖ±»±ğÈËÈ¡×ß£¬ĞèÒª°´Ê£ÓàÊ±¼äÖØÊÔ */
    start_tick = svcrt_kernel_tick;
    deadline   = (timeout_ms > 0) ? SVCRT_MS_TO_TICK((uint32)timeout_ms) : 0u;
    ...
    /* ±»»½ĞÑ²»µÈÓÚÒ»¶¨ÄÃµ½ÏûÏ¢£º»½ĞÑºóÏûÏ¢¿ÉÄÜÒÑ±»ÆäËü½ÓÊÕÕßÈ¡×ß¡£
     * Ô­ÊµÏÖ´ËÊ±Ö±½Ó·µ»Ø -1£¨¶ÓÁĞÀïÃ÷Ã÷ÓĞÊı¾İÈ´±»ÅĞÊ§°Ü£©£»ÕâÀï°´Ê£ÓàÊ±¼äÖØÊÔ¡£ */
    while(p_mq->count <= 0)
    {
        remain = svcrt_mq_remain_ms(start_tick, deadline);
        if(remain == 0)
        ...
        reason = svcrt_task_block_in_critical((uint32)remain);  /* ¹ØÖĞ¶Ï·µ»Ø */
