...
- **栈溢出检测**：通过栈底标志位检测任务栈溢出
- **CPU 负载统计**：实时统计 CPU 空闲率
- **FPU 支持**：Cortex-M4F/M7 浮点寄存器自动保存/恢复
- **双 SDK 架构**：独立的应用 SDK 和驱动 SDK
...
3. **实现 `svcrt_board.c`**：实现 `svcrt_port.h` 中的所有接口函数 + 中断入口
4. **实现 `svcrt_mpu.c`**：MPU 操作（M3 可跳过）
5. **实现上下文切换汇编**：`context_rvds.S`
6. **编写板载驱动**：UART、LED 等
